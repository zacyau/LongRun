<!--
 * GrowthValueView.vue - 成长100/价值100 风格轮动分析页面
 *
 * 本页面对比分析成长100与价值100的相对表现，
 * 帮助投资者了解成长风格与价值风格的轮动节奏。
 *
 * 包含四个核心图表：
 * 1. 收益走势对比：成长100 vs 价值100
 * 2. 成长/价值比值布林线：分析比值的波动区间和位置
 * 3. 40日收益差：短期超额收益分析
      ...
      <div class="header-inner">
        <div class="header-brand">
          <div class="brand-info">
            <h1 class="header-title">成长100 <span class="title-sep">/</span> 价值100</h1>
            <p class="header-subtitle">风格轮动镜 · 成长100相对价值100的超额收益分析</p>
          </div>
        </div>
        ...
        <div class="chart-card">
          <div class="card-meta">
            ...
            <div class="card-legend">
              <span class="legend-item legend-growth">
                <span class="legend-dot"></span>成长100
              </span>
              <span class="legend-item legend-value">
                <span class="legend-dot"></span>价值100
              </span>
            </div>
          </div>
        ...
        <div class="chart-card">
          <div class="card-meta">
            <div class="card-title-group">
              <h3 class="card-title">成长/价值 比值布林线</h3>
        ...
        <div class="chart-card">
          <div class="card-meta">
            <div class="card-title-group">
              ...
              <span class="card-desc">成长100 − 价值100（MA242）</span>
            </div>
            <div class="card-actions">
              <div class="card-badges" v-if="store.data">
        ...
        <div v-if="showModal2" class="modal-overlay" @click.self="showModal2 = false">
          <div class="modal-panel">
            ...
            <div class="modal-body">
              ...
              <div class="modal-section">
                <h4>作用</h4>
                <p>判断成长/价值比值处于历史波动区间的什么位置。%B靠近0说明比值接近下轨（成长相对偏弱），靠近1说明比值接近上轨（成长相对偏强）。带宽变宽说明波动加大。</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="modal-panel">
            ...
            <div class="modal-body">
              <div class="modal-section">
                <h4>计算</h4>
                <ul class="modal-list">
                  <li>40日收益差 = 成长100 40日累计收益率 - 价值100 40日累计收益率</li>
                  <li>再叠加一条242日均线（MA242）</li>
                </ul>
              </div>
              <div class="modal-section">
                <h4>作用</h4>
                <p>衡量短期（40日）内成长100相对价值100的超额收益方向和幅度。差值为正说明成长短期跑赢，反之跑输。均线方向反映中期趋势。</p>
        ...
        <div v-if="showModal4" class="modal-overlay" @click.self="showModal4 = false">
          <div class="modal-panel">
            ...
            <div class="modal-body">
              ...
              <div class="modal-section">
                <h4>作用</h4>
                <p>RSI反映比值的动能强弱。RSI&gt;70说明比值处于强势上涨区间（成长相对强势），RSI&lt;30说明比值弱势（成长相对跑输）。和图2结合可以看价格位置+动能方向。</p>
              </div>
            </div>
          </div>
        </div>
...
import { useGrowthValueStore } from '@/stores/growthValueStore'
...
const store = useGrowthValueStore()
...
const chart1Option = computed(() => {
  ...
  return {
    ...
    series: [
      { name: '成长100', type: 'line', data: d.growth, smooth: false, symbol: 'none', lineStyle: { width: 1.5, color: COLORS.growth } },
      { name: '价值100', type: 'line', data: d.value, smooth: false, symbol: 'none', lineStyle: { width: 1.5, color: COLORS.value } },
    ],
  }
})
...
.legend-growth .legend-dot { background: #FF6D00; }
.legend-value .legend-dot { background: #00897B; }
...
@media (max-width: 768px) {
  .gv-header { padding: var(--space-3) var(--page-padding-mobile); }
  .header-inner { flex-direction: column; align-items: flex-start; }
  .header-title { font-size: var(--text-xl); }
  .gv-main { padding: var(--space-3) var(--page-padding-mobile); }
  .charts-grid {
    gap: 10px;
  }
